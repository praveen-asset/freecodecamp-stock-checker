...
suite('Functional Tests', function() {
    suite('5 functions test', function() {
        test('Viewing one stock: GET request to /api/stock-prices/', function(done) {
            ...
            .query({stock: 'GOOG'}).end(function(err, res){
        ...
        test('Viewing one stock and liking it: GET request to /api/stock-prices/', function(done) {
            chai.request(server).get('/api/stock-prices/').set('content-type', 'application/json')
            .query({stock: 'GOLD', like: true}).end(function(err, res){
                assert.equal(res.status, 200)
        ...
        test('Viewing the same stock and liking it again: GET request to /api/stock-prices/', function(done) {
            chai.request(server).get('/api/stock-prices/').set('content-type', 'application/json')
            .query({stock: 'GOLD'}).end(function(err, res){
                assert.equal(res.status, 200)
                assert.equal(res.body.stockData.stock, 'GOLD')
                assert.equal(res.body.stockData.likes, 1)
                assert.exists(res.body.stockData.price, 'GOLD has a price')
        ...
        test('Viewing two stocks: GET request to /api/stock-prices/', function(done) {
            chai.request(server).get('/api/stock-prices/').set('content-type', 'application/json')
            .query({stock: ['MSFT', 'AMZN']}).end(function(err, res){
                assert.equal(res.status, 200)
        ...
        test('Viewing two stocks and liking them: GET request to /api/stock-prices/', function(done) {
            chai.request(server).get('/api/stock-prices/').set('content-type', 'application/json')
            .query({stock: ['MSFT', 'AMZN'], like: true}).end(function(err, res){
                assert.equal(res.status, 200)
                assert.equal(res.body.stockData[0].stock, 'MSFT')
                assert.equal(res.body.stockData[1].stock, 'AMZN')
                assert.exists(res.body.stockData[0].price, 'MSFT has a price')
